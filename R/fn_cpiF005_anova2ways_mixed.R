...
fn_cpiF005_TakeCode <- function(selected_fn){
...
fn_cpiF005_ObjNamesInOrder <- function(selected_fn){
...
# # # # Control funcitons
# Control previous
fn_cpiF005_control_previous <- function(database, vr_var_name, factor01_var_name, factor02_var_name, alpha_value){
  ...
  # 15) factor01_var_name is a colname from database
  ...
  # # # # # # # # factor01_var_name
  # 10) factor01_var_name is not NULL
  if(is.null(factor01_var_name)){
    text_output <- "Control pre test 011: Object 'factor01_var_name' can not be NULL."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # 11) factor01_var_name is a vector
  if(!is.vector(factor01_var_name)){
    text_output <- "Control pre test 012: Object 'factor01_var_name' must be vector."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # 12) factor01_var_name is a vector
  if(!(length(factor01_var_name) == 1)){
    text_output <- "Control pre test 013: Object 'factor01_var_name' must be vector of length 1."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # 13) factor01_var_name is not NA
  if(is.na(factor01_var_name)){
    text_output <- "Control pre test 014: Object 'factor01_var_name' can not be NA."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # 14) factor01_var_name is character
  if(!is.character(factor01_var_name)){
    text_output <- "Control pre test 015: Object 'factor01_var_name' must be character."
    return(Hmisc::llist(dt_ok, text_output))
  }


  # 15) factor01_var_name is a colname from database
  if(!(factor01_var_name %in% colnames(database))){
    text_output <- "Control pre test 016: Object 'factor01_var_name' must be a colname from database."
    return(Hmisc::llist(dt_ok, text_output))
  }
  ...
  # # # # # # # # factor01_var_name
  # 10) factor01_var_name is not NULL
  if(is.null(factor02_var_name)){
    text_output <- "Control pre test 011: Object 'factor02_var_name' can not be NULL."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # 11) factor01_var_name is a vector
  if(!is.vector(factor02_var_name)){
    text_output <- "Control pre test 012: Object 'factor02_var_name' must be vector."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # 12) factor01_var_name is a vector
  if(!(length(factor02_var_name) == 1)){
    text_output <- "Control pre test 013: Object 'factor02_var_name' must be vector of length 1."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # 13) factor01_var_name is not NA
  if(is.na(factor02_var_name)){
    text_output <- "Control pre test 014: Object 'factor02_var_name' can not be NA."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # 14) factor01_var_name is character
  if(!is.character(factor02_var_name)){
    text_output <- "Control pre test 015: Object 'factor02_var_name' must be character."
    return(Hmisc::llist(dt_ok, text_output))
  }


  # 15) factor01_var_name is a colname from database
  if(!(factor02_var_name %in% colnames(database))){
    text_output <- "Control pre test 016: Object 'factor02_var_name' must be a colname from database."
    return(Hmisc::llist(dt_ok, text_output))
  }
  ##########
  ...
  # # # # # # # # factor01_var_name and vr_var_name
  # 15) factor01_var_name is not NULL
  if(vr_var_name == factor01_var_name){
    text_output <- "Control pre test 023: Objects 'vr_var_name' and 'factor01_var_name' can not be equal."
    return(Hmisc::llist(dt_ok, text_output))
  }

  if(vr_var_name == factor02_var_name){
    text_output <- "Control pre test 023: Objects 'vr_var_name' and 'factor02_var_name' can not be equal."
    return(Hmisc::llist(dt_ok, text_output))
  }

  if(factor01_var_name == factor02_var_name){
    text_output <- "Control pre test 023: Objects 'factor01_var_name' and 'factor02_var_name' can not be equal."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # New object
  vector_var_names <- c(vr_var_name, factor01_var_name, factor02_var_name)

  if(sum(vector_var_names %in% colnames(database)) != 3){
    text_output <- "Control pre test 024: Objects 'vr_var_name', 'factor01_var_name' and 'factor02_var_name' must be colnames from database."
    return(Hmisc::llist(dt_ok, text_output))
  }
  ...
  colnames(minibase) <- c("RV", "FACTOR01", "FACTOR02")
  ...
  if(!(nlevels(minibase$FACTOR01) >= 2)){
    text_output <- "Control pre test 033: for Anova test FACTOR01 must has at least 2 levels."
    return(Hmisc::llist(dt_ok, text_output))
  }

  # Al least 2 levels in minibase$FACTOR
  if(!(nlevels(minibase$FACTOR02) >= 2)){
    text_output <- "Control pre test 033: for Anova test FACTOR02 must has at least 2 levels."
    return(Hmisc::llist(dt_ok, text_output))
  }
...
# Control post
fn_cpiF005_control_post <- function(list_results_from_fn_cpiF005){

  # if(is.null(list_results_from_fn_cpiF005)){
  #   text_output <- "Control post test 001: Object 'list_results_from_fn_cpiF005' can not be NULL."
  #   check_ok <- FALSE
  ...
  #
  #
  # if(!(obj_name01 %in% names(list_results_from_fn_cpiF005))){
  #   text_output <- "Control post test 002: Object 'df_table_anova' doesn't exist in 'list_results_from_fn_cpiF005'."
  #   check_ok <- FALSE
  #   return(Hmisc::llist(check_ok, text_output))
  # }
  #
  #
  # # # # 1) About the table
  # if(is.null(list_results_from_fn_cpiF005[obj_name01])){
  ...
  #
  #
  # selected_obj01 <- list_results_from_fn_cpiF005[[obj_name01]]
  #
  #
...
fn_cpiF005_results <- function(database, vr_var_name, factor01_var_name, factor02_var_name, alpha_value){
  ...
  vector_name_selected_vars <- c(vr_var_name, factor01_var_name, factor02_var_name)
  vector_rol_vars <- c("RV", "FACTOR01", "FACTOR02")
  ...
  # on Tukey.
  check_unbalanced_reps_factor01 <- length(unique(df_factor01_info$n)) > 1
  check_unbalanced_reps_factor02 <- length(unique(df_factor02_info$n)) > 1
  check_unbalanced_reps_combination <- length(unique(df_combination_info$n)) > 1



  # https://people.math.ethz.ch/~meier/teaching/anova/random-and-mixed-effects-models.html#more-than-one-factor

  confidence_value <- 1 - alpha_value

  list_lmer_anova <- lmerTest::lmer(RV ~ FACTOR01 + (1 | FACTOR02) + (1 | FACTOR01:FACTOR02) , data = minibase)

  intervalo_confianza <- suppressMessages(confint(list_lmer_anova,
                                                  oldNames = FALSE,
                                                  level = confidence_value))

  #options(contrasts = c("contr.treatment", "contr.poly"))
  list_fixed_efect <- stats::anova(object = list_lmer_anova)

  summary_lmer_anova <- summary(list_lmer_anova)

  list_anova <- lmerTest::ranova(list_lmer_anova)


  # Obtener los valores ajustados (emmeans) para el factor fijo
  emmeans_modelo <- emmeans(list_lmer_anova, ~ FACTOR01)

  # Realizar la prueba de Tukey HSD
  tukey_result <- pairs(emmeans_modelo, adjust = "tukey")

  # Obtener el resumen de los resultados
  summary(tukey_result)


  modelo_aov <- aov(RV ~ FACTOR01 + Error(FACTOR02), data = minibase)
  tukey_hsd <- agricolae::HSD.test(modelo_aov, "FACTOR01", group = TRUE)
  ...
  tukey_factor01_full_groups <- agricolae::HSD.test(y = list_lmer_anova,
                                                    trt = "FACTOR01",
                                                    alpha = alpha_value,
                                                    group = TRUE,
                                                    console = FALSE,
                                                    unbalanced = check_unbalanced_reps_factor01)



  # # # Tukey test - Tukey pairs comparation - Full version
  tukey_factor01_full_pairs <- agricolae::HSD.test(y = lm_anova,
                                                   trt = "FACTOR02",
                                                   alpha = alpha_value,
                                                   group = FALSE,
                                                   console = FALSE,
  ...
  df_tukey_table_factor01 <- tukey_factor01_full_groups$groups
  df_tukey_table_factor01


  # Extraer las varianzas
  df_info <- as.data.frame(lme4::VarCorr(list_lmer_anova))
  df_info$grp[3] <- "ERROR"
  vector_names <- c(df_info$grp, "TOTAL")
  vector_variances <- c(df_info$vcov, sum(df_info$vcov))
  vector_sd <- sqrt(vector_variances)
  names(vector_variances) <- vector_names
  names(vector_sd) <- vector_names

  vector_prop_variances <- vector_variances/vector_variances["TOTAL"]
  vector_porc_variances <- vector_prop_variances*100
  model_error_sd <- vector_sd["ERROR"]

  # Crear un dataframe con las varianzas del factor aleatorio y del error
  df_variances <- data.frame(
    "order" = 1:length(vector_variances),
    "fount_of_variance" = names(vector_variances),
    "sd" = vector_sd,
    "variance" = vector_variances,
    "proportional_variance" = vector_prop_variances,
    "percentage_variance" = vector_porc_variances
  )


  # # # Detect rows on database there are on minibase
  dt_rows_database_ok <- rowSums(!is.na(database[vector_name_selected_vars])) == length(  vector_rol_vars)



  # # # Object minibase_mod and new cols
  minibase_mod <- minibase
  minibase_mod$"order_number_lvl_factor01" <- as.numeric(minibase_mod[,2])
  minibase_mod$"color_lvl_factor01" <- df_factor01_info$color[minibase_mod$"order_number_lvl_factor01"]
  minibase_mod$"order_number_lvl_factor02" <- as.numeric(minibase_mod[,3])
  minibase_mod$"color_lvl_factor02" <- df_factor02_info$color[minibase_mod$"order_number_lvl_factor02"]
  ...
  minibase_mod$"residuals" <- residuals(list_lmer_anova)
  ...
  list_normality_test <- shapiro.test(minibase_mod$"residuals")

  list_homogeneity_test <- bartlett.test(residuals ~ COMBINATION, data = minibase_mod)

  # # # Residuals variance from levels from original residuals
  df_residuals_variance_combination_levels <- data.frame(
    "order" = 1:nlevels(minibase_mod[,4]),
    "factor" = levels(minibase_mod[,4]),
    "variance" = tapply(minibase_mod$residuals, minibase_mod[,4], var),
    "n" = tapply(minibase_mod$residuals, minibase_mod[,4], length)
  )
  df_residuals_variance_combination_levels
  ...
  # # # # # Section 10 - Partitioned Measures (VR) for FACTOR01 --------------------------------
  # # # Partitioned Measures Position (VR) for FACTOR
  df_vr_position_levels_factor01 <- data.frame(
    "order" = 1:nlevels(minibase[,2]),
    "level" = levels(minibase[,2]),
  ...
  df_vr_position_levels_factor01


  # # # Partitioned Measures of Dispersion (VR) for FACTOR
  df_vr_dispersion_levels_factor01 <- data.frame(
    "order" = 1:nlevels(minibase[,2]),
    "level" = levels(minibase[,2]),
    "n" = tapply(minibase[,1], minibase[,2], length),
  ...
  df_vr_dispersion_levels_factor01
  ...
  # # # Partitioned Measures Position (VR) for FACTOR
  df_vr_position_levels_factor02 <- data.frame(
    "order" = 1:nlevels(minibase[,3]),
    "level" = levels(minibase[,3]),
  ...
  df_vr_position_levels_factor02


  # # # Partitioned Measures of Dispersion (VR) for FACTOR
  df_vr_dispersion_levels_factor02 <- data.frame(
    "order" = 1:nlevels(minibase[,3]),
    "level" = levels(minibase[,3]),
    "n" = tapply(minibase[,1], minibase[,3], length),
  ...
  df_vr_dispersion_levels_factor02
  ...
  df_residuals_position_levels_combination <- data.frame(
    "order" = 1:nlevels(minibase_mod[,4]),
    "level" = levels(minibase_mod[,4]),
    "min" = tapply(minibase_mod$residuals, minibase_mod[,4], min),
    "mean" = tapply(minibase_mod$residuals, minibase_mod[,4], mean),
    "median" = tapply(minibase_mod$residuals, minibase_mod[,4], median),
    "max" = tapply(minibase_mod$residuals, minibase_mod[,4], max),
    "n" = tapply(minibase_mod$residuals, minibase_mod[,4], length)
  )
  df_residuals_position_levels_combination



  # # # Partitioned Measures of Dispersion (residuals) for FACTOR ----------------
  df_residual_dispersion_levels_combination <- data.frame(
    "order" = 1:nlevels(minibase_mod[,4]),
    "level" = levels(minibase_mod[,4]),
    "range" = tapply(minibase_mod$residuals, minibase_mod[,4], function(x){max(x) - min(x)}),
    "variance" = tapply(minibase_mod$residuals, minibase_mod[,4], var),
    "standard_deviation" = tapply(minibase_mod$residuals, minibase_mod[,4], sd),
    "standard_error" = tapply(minibase_mod$residuals, minibase_mod[,4], function(x){sd(x)/sqrt(length(x))}),
    "n" = tapply(minibase_mod$residuals, minibase_mod[,4], length)
  )
  df_residual_dispersion_levels_combination
  ...
  hide_correct_order <- fn_cpiF005_ObjNamesInOrder(selected_fn = fn_cpiF005_results)
...
fn_cpiF005_code_section01_Libreries <- function(){
...
fn_cpiF005_code_section02_FileSource <- function(intro_source_database){
...
fn_cpiF005_code_section03_VarSelection <- function(vr_var_name, factor01_var_name, alpha_value){
  ...
  factor01_var_name <- "_selected_factor01_var_name_"
  ...
  section03_varselection <- gsub(pattern = "_selected_factor01_var_name_" ,
                                 replacement =  factor01_var_name,
                                 x = section03_varselection)
...
fn_cpiF005_code_section04_UntilTheEnd <- function(){


  the_code <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_results)

  return(the_code)
}


fn_cpiF005_code_sectionXX_g01_Plots <- function(){



  list_code <- list()

  list_code[[1]] <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_factor_plot001)
  list_code[[2]] <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_factor_plot002)
  list_code[[3]] <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_factor_plot003)
  list_code[[4]] <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_factor_plot004)
  list_code[[5]] <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_factor_plot005)
  list_code[[6]] <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_factor_plot006)
  list_code[[7]] <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_factor_plot007)
...
fn_cpiF005_code_sectionXX_g02_Plots <- function(){


  list_code <- list()

  list_code[[1]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot001)
  list_code[[2]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot002)
  list_code[[3]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot003)
  list_code[[4]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot004)
  list_code[[5]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot005)
  list_code[[6]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot006)
  list_code[[7]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot007)
  list_code[[8]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot008)
  list_code[[9]]  <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot009)
  list_code[[10]] <- fn_cpiF005_TakeCode(selected_fn = fn_cpiF005_residuals_plot010)
  ...
  # objetos <- ls("package:Rscience", pattern = "^fn_cpiF005_residuals_plot+[0-9]+")
  #
  #
  #
  # # Filtrar solo las funciones que coinciden con el patrón
  # #list_plot <- objetos[grep("^fn_cpiF005_plot[0-9]+", objetos)]
  # list_plot <- objetos
  ...
  #
  #
  #   selected_code <- paste0("fn_cpiF005_TakeCode(", x, ")")
...
fn_cpiF005_code_sectionALL <- function(intro_source_database, vr_var_name, factor01_var_name, alpha_value){

  # objetos <- ls()
  #
  # # Filtrar solo las funciones que coinciden con el patrón
  # selected_fun <- objetos[grep("^fn_cpiF005_code_section[0-9]+_[a-zA-Z]", objetos)]
  # selected_fun <- sort(selected_fun)

  vector_code <- list()

  vector_code[1] <- fn_cpiF005_code_section01_Libreries()
  vector_code[2] <- fn_cpiF005_code_section02_FileSource(intro_source_database)
  vector_code[3] <- fn_cpiF005_code_section03_VarSelection(vr_var_name, factor01_var_name, alpha_value)
  vector_code[4] <- fn_cpiF005_code_section04_UntilTheEnd()
  vector_code[5] <- fn_cpiF005_code_sectionXX_g01_Plots()
  vector_code[6] <- fn_cpiF005_code_sectionXX_g02_Plots()
...
fn_cpiF005_recruit_g01_Tables <- function(list_results_from_fn_cpiF005){


  all_tables <- with(list_results_from_fn_cpiF005, {
...
fn_cpiF005_recruit_g02_Tables <- function(list_results_from_fn_cpiF005){
...
# # # Plots FACTOR
fn_cpiF005_factor_plot001 <- function(minibase_mod, df_factor_info){
...
fn_cpiF005_factor_plot002 <- function(df_table_factor_plot002){
...
fn_cpiF005_factor_plot003 <- function(df_table_factor_plot003){
...
fn_cpiF005_factor_plot004 <- function(df_table_factor_plot004){
...
fn_cpiF005_factor_plot005 <- function(minibase_mod, df_table_factor_plot005){
...
fn_cpiF005_factor_plot006 <- function(minibase_mod, df_table_factor_plot006){
...
fn_cpiF005_factor_plot007 <- function(df_table_factor_plot007){
...
fn_cpiF005_recruit_g01_FactorPlots <- function(list_results_from_fn_cpiF005){



  all_plots <-  with(list_results_from_fn_cpiF005, {

    list_plots <- list()

    list_plots[[1]] <- fn_cpiF005_factor_plot001(minibase_mod, df_factor_info)
    list_plots[[2]] <- fn_cpiF005_factor_plot002(df_table_factor_plot002)
    list_plots[[3]] <- fn_cpiF005_factor_plot003(df_table_factor_plot003)
    list_plots[[4]] <- fn_cpiF005_factor_plot004(df_table_factor_plot004)
    list_plots[[5]] <- fn_cpiF005_factor_plot005(minibase_mod, df_table_factor_plot005)
    list_plots[[6]] <- fn_cpiF005_factor_plot006(minibase_mod, df_table_factor_plot006)
    list_plots[[7]] <- fn_cpiF005_factor_plot007(df_table_factor_plot007)
...
fn_cpiF005_residuals_plot001 <- function(minibase_mod, df_factor_info){
...
fn_cpiF005_residuals_plot002 <- function(minibase_mod, df_table_residuals_plot002){
...
fn_cpiF005_residuals_plot003 <- function(minibase_mod, df_table_residuals_plot003){
...
fn_cpiF005_residuals_plot004 <- function(minibase_mod){
...
fn_cpiF005_residuals_plot005 <- function(minibase_mod){
...
fn_cpiF005_residuals_plot006 <- function(minibase_mod, df_factor_info){
...
fn_cpiF005_residuals_plot007 <- function(minibase_mod, df_factor_info){
...
fn_cpiF005_residuals_plot008 <- function(minibase_mod){
...
fn_cpiF005_residuals_plot009 <- function(minibase_mod){
...
fn_cpiF005_residuals_plot010 <- function(minibase_mod){
...
fn_cpiF005_recruit_g02_ResidualsPlots <- function(list_results_from_fn_cpiF005){
  ...
  all_plots <-  with(list_results_from_fn_cpiF005, {
    ...
    list_plots[[1]]  <- fn_cpiF005_residuals_plot001(minibase_mod, df_factor_info)
    list_plots[[2]]  <- fn_cpiF005_residuals_plot002(minibase_mod, df_table_residuals_plot002)
    list_plots[[3]]  <- fn_cpiF005_residuals_plot003(minibase_mod, df_table_residuals_plot003)
    list_plots[[4]]  <- fn_cpiF005_residuals_plot004(minibase_mod)
    list_plots[[5]]  <- fn_cpiF005_residuals_plot005(minibase_mod)
    list_plots[[6]]  <- fn_cpiF005_residuals_plot006(minibase_mod, df_factor_info)
    list_plots[[7]]  <- fn_cpiF005_residuals_plot007(minibase_mod, df_factor_info)
    list_plots[[8]]  <- fn_cpiF005_residuals_plot008(minibase_mod)
    list_plots[[9]]  <- fn_cpiF005_residuals_plot009(minibase_mod)
    list_plots[[10]] <- fn_cpiF005_residuals_plot010(minibase_mod)
