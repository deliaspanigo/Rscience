...
module_cpiA003_s01_varselection_ui <- function(id){
...
module_cpiA003_s01_varselection_server <- function(id, input_general){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$vars_selection <- renderUI({
        ...
        div(shinyjs::useShinyjs(), id = ns("input-var-selection"),
            ...
            fluidRow(
              ...
              column(2,
                     selectInput(inputId = ns("factor01_var_name"), label = "Factor01",
                                 choices = set_options,
                                 selected = set_options[1])
              ),
              column(2,
                     selectInput(inputId = ns("factor02_var_name"), label = "Factor02",
                                 choices = set_options,
                                 selected = set_options[1])
              ),
      ...
      control_user_02 <- reactive({
        ...
        validate(
          ...
          need(!is.null(input$factor01_var_name), "Error 10: Module anova s01 - input$factor01_var_name can not be NULL."),
          need(!is.null(input$factor02_var_name), "Error 10: Module anova s01 - input$factor02_var_name can not be NULL."),
        ...
        validate(
          ...
          need(is.vector(input$factor01_var_name), "Error 13: Module anova s01 - input$factor01_var_name must be a vector."),
          need(is.vector(input$factor02_var_name), "Error 13: Module anova s01 - input$factor02_var_name must be a vector."),
        ...
        validate(
          ...
          need(length(input$factor01_var_name) == 1, "Error 16: Module anova s01 - input$factor01_var_name has length 1."),
          need(length(input$factor02_var_name) == 1, "Error 16: Module anova s01 - input$factor02_var_name has length 1."),
...
module_cpiA003_s02_rscience_ui <- function(id){
...
module_cpiA003_s02_rscience_server <- function(id, input_general, input_01_anova){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      control_user_01 <- reactive({
        ...
        # )
        #
        # check_previous <- fn_cpiA003_control_previous(database = input_general()$database,
      ...
      RR_general <- reactive({
        ...
        the_output <- fn_cpiA003_gen02(database = input_general()$database,
                                       vr_var_name = input_01_anova()$vr_var_name,
                                       factor01_var_name = input_01_anova()$factor01_var_name,
                                       factor02_var_name = input_01_anova()$factor02_var_name,
                                       alpha_value = input_01_anova()$alpha_value)
      ...
      control_user_02 <- reactive({
        ...
        # #
        # #         # Control post
        # check_post <- fn_cpiA003_control_p01_test(all_results = RR_general()$"R_results"$"p01_test")
        # #
        # validate(
