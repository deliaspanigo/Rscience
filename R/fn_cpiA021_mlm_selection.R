...
fn_cpiA021_TakeCode <- function(selected_fn){
...
fn_cpiA021_ObjNamesInOrder <- function(selected_fn){
...
# # # # Control funcitons
# Control previous
fn_cpiA021_control_previous <- function(database, vr_var_name, x_var_name, alpha_value){
...
fn_cpiA021_code_p01_test <- function(database, vr_var_name, x_var_name, alpha_value){
  ...
  # # # Anova test
  amount_reg_vars <- length(x_var_name)
  amount_models <- 2^amount_reg_vars

  phrase01 <- paste0("Amount of regresion variables: k = ", amount_reg_vars, ".")

  phrase02 <- "Amount of models: 2^k = 2^_amount_reg_vars_ = _amount_models_."
  phrase02 <- gsub(pattern = "_amount_reg_vars_", replacement = amount_reg_vars, x = phrase02)
  phrase02 <- gsub(pattern = "_amount_models_", replacement = amount_models, x = phrase02)

  phrase03 <- paste0("Between the null model and the full model there are ", amount_models, " different possible models.")


  model_null_lm <- lm(VR ~ 1, data = minibase)
  model_full_lm <- lm(VR ~ ., data = minibase)

  print_forward_lm <- capture.output(step(model_null_lm, scope = list(lower = model_null_lm , upper = model_full_lm),
                                          direction="forward"))

  select_forward_lm <- step(model_null_lm, scope = list(lower = model_null_lm , upper = model_full_lm),
                            direction="forward", trace = 0)

  list_summary_forward <- summary(select_forward_lm)

  df_model_forward <- as.data.frame(list_summary_forward$coefficients)


  df_table_det_coef_forward <- data.frame(
    "r.squared" =  list_summary_forward$r.squared,
    "adj.r.squared" = list_summary_forward$adj.r.squared,
    "AIC" = AIC(select_forward_lm)
  )
  #df_table_det_coef_forward$"p.value" <- "No sale siempre"
  # df_table_det_coef_forward$"p.value" <- pf(q = df_table_det_coef_forward$"f.obs",
  #                                           df1 = df_table_det_coef_forward$"df_num",
  #                                           df2 = df_table_det_coef_forward$"df_den")
  rownames(df_table_det_coef_forward) <- rep("", nrow(df_table_det_coef_forward))



  # Backward
  print_backward_lm <- capture.output(step(model_full_lm, scope = list(lower = model_null_lm, upper = model_full_lm),
                                           direction="backward"))


  select_backward_lm <- step(model_full_lm, scope = list(lower = model_null_lm, upper = model_full_lm),
                             direction="backward", trace = 0)

  list_summary_backward <- summary(select_backward_lm)

  df_model_backward <- as.data.frame(list_summary_backward$coefficients)

  df_table_det_coef_backward <- data.frame(
    "r.squared" =  list_summary_backward$r.squared,
    "adj.r.squared" = list_summary_backward$adj.r.squared,
    "AIC" = AIC(select_backward_lm)
  )

  #df_table_det_coef_backward$"p.value" <- "No sale siempre"
  # df_table_det_coef_backward$"p.value" <- pf(q = df_table_det_coef_backward$"f.obs",
  #                                   df1 = df_table_det_coef_backward$"df_num",
  #                                   df2 = df_table_det_coef_backward$"df_den")
  rownames(df_table_det_coef_backward) <- rep("", nrow(df_table_det_coef_backward))
  ...
  minibase_mod_forward <- minibase
  minibase_mod_forward$"fitted.values" <- select_forward_lm$fitted.values
  minibase_mod_forward$"residuals" <- select_forward_lm$residuals
  minibase_mod_forward$"studres" <- minibase_mod_forward$"residuals"/sd(minibase_mod_forward$"residuals")
  minibase_mod_forward$"id_database" <- c(1:nrow(database))[dt_rows_database_ok]
  minibase_mod_forward$"id_minibase" <- 1:nrow(minibase)




  minibase_mod_backward <- minibase
  minibase_mod_backward$"fitted.values" <- select_backward_lm$fitted.values
  minibase_mod_backward$"residuals" <- select_backward_lm$residuals
  minibase_mod_backward$"studres" <- minibase_mod_backward$"residuals"/sd(minibase_mod_backward$"residuals")
  minibase_mod_backward$"id_database" <- c(1:nrow(database))[dt_rows_database_ok]
  minibase_mod_backward$"id_minibase" <- 1:nrow(minibase)
  ...
  test_residuals_normality_forward <- shapiro.test(minibase_mod_forward$residuals)
  test_residuals_normality_forward

  test_residuals_normality_backward <- shapiro.test(minibase_mod_backward$residuals)
  test_residuals_normality_backward
  ...
  list_position_forward <- lapply(1:length(detail_role), function(x){
    ...
    data.frame(
      ...
      "n" = length(minibase_mod_forward[,selected_role]),
      "min" = min(minibase_mod_forward[,selected_role]),
      "mean" = mean(minibase_mod_forward[,selected_role]),
      "median" = median(minibase_mod_forward[,selected_role]),
      "max" = max(minibase_mod_forward[,selected_role])
    )
  })
  df_position_forward <- do.call(rbind.data.frame, list_position_forward)
  df_position_forward




  list_dispersion_forward <- lapply(1:length(detail_role), function(x){
    ...
    data.frame(
      ...
      "var_name" = detail_name[x],
      "n" = length(minibase_mod_forward[,selected_role]),
      "range" = max(minibase_mod_forward[,selected_role]) - min(minibase_mod_forward[,selected_role]),
      "variance" = var(minibase_mod_forward[,selected_role]),
      "sd" = sd(minibase_mod_forward[,selected_role])
    )
  })
  df_dispersion_forward <- do.call(rbind.data.frame, list_dispersion_forward)
  df_dispersion_forward

  ######
  list_position_backward <- lapply(1:length(detail_role), function(x){
    ...
    data.frame(
      "rol_var" = selected_role,
      "var_name" = detail_name[x],
      "n" = length(minibase_mod_backward[,selected_role]),
      "min" = min(minibase_mod_backward[,selected_role]),
      "mean" = mean(minibase_mod_backward[,selected_role]),
      "median" = median(minibase_mod_backward[,selected_role]),
      "max" = max(minibase_mod_backward[,selected_role])
    )
  })
  df_position_backward <- do.call(rbind.data.frame, list_position_backward)
  df_position_backward




  list_dispersion_backward <- lapply(1:length(detail_role), function(x){
    ...
    data.frame(
      ...
      "var_name" = detail_name[x],
      "n" = length(minibase_mod_backward[,selected_role]),
      "range" = max(minibase_mod_backward[,selected_role]) - min(minibase_mod_backward[,selected_role]),
      "variance" = var(minibase_mod_backward[,selected_role]),
      "sd" = sd(minibase_mod_backward[,selected_role])
    )
  })
  df_dispersion_backward <- do.call(rbind.data.frame, list_dispersion_backward)
  df_dispersion_backward
  ###########


  # Tabla plot 001
  df_table_plot001 <- df_position_forward


  # Tabla plot 002
  df_table_plot002 <- df_position_forward



  df_table_plot003 <- df_position_backward


  df_table_plot004 <- df_position_backward
  ...
  hide_correct_order <- fn_cpiA021_ObjNamesInOrder(selected_fn = fn_cpiA021_code_p01_test)
...
fn_cpiA021_code_p02_plot001 <- function(results_p01_test){
...
fn_cpiA021_code_p02_plot002 <- function(results_p01_test){
  ...
  new_plot <-  with(results_p01_test,{
    ...
    plot002 <- add_trace(p = plot002,
                         x = minibase_mod_forward$fitted.values,
                         y = minibase_mod_forward$residuals,
                         type = 'scatter',
                         mode = 'markers',
                         name = "data",
                         marker = list(size = 15, color = 'blue'))
    ...
    plot002 <- plotly::layout(p = plot002,
                              ...
                              title = "Plot 001 - Forward - Residuals vs. Fitted values",
                              font = list(size = 20),
                              margin = list(t = 100))
...
fn_cpiA021_code_p02_plot003 <- function(results_p01_test){
  ...
  new_plot <-  with(results_p01_test,{
    ...
    plot003 <- add_trace(p = plot003,
                         x = minibase_mod_backward$fitted.values,
                         y = minibase_mod_backward$residuals,
                         type = 'scatter',
                         mode = 'markers',
                         name = "data",
                         marker = list(size = 15, color = 'blue'))
    ...
    plot003 <- plotly::layout(p = plot003,
                              xaxis = list(title = "Fitted values"),
                              yaxis = list(title = "Residuals"),
                              title = "Plot 002 - Backward - Residuals vs. Fitted values",
                              font = list(size = 20),
                              margin = list(t = 100))
...
fn_cpiA021_code_p02_plot004 <- function(results_p01_test){
...
# Control post
fn_cpiA021_control_p01_test <- function(all_results){
...
fn_cpiA021_control_p02_plots <- function(all_results){
...
fn_cpiA021_gen01 <- function(database,  vr_var_name, x_var_name, alpha_value){
  ...
  # Step 01 - R_code
  output_list$"R_code"$"p01_test"    <- fn_cpiA021_TakeCode(selected_fn = fn_cpiA021_code_p01_test)
  output_list$"R_code"$"p02_plots"$"plot001"   <- fn_cpiA021_TakeCode(selected_fn = fn_cpiA021_code_p02_plot001)
  output_list$"R_code"$"p02_plots"$"plot002"   <- fn_cpiA021_TakeCode(selected_fn = fn_cpiA021_code_p02_plot002)
  output_list$"R_code"$"p02_plots"$"plot003"   <- fn_cpiA021_TakeCode(selected_fn = fn_cpiA021_code_p02_plot003)
  output_list$"R_code"$"p02_plots"$"plot004"   <- fn_cpiA021_TakeCode(selected_fn = fn_cpiA021_code_p02_plot004)


  # Step 02 - Pre Control ------------------------------------------------------
  output_list$"check_control"$"previous" <- fn_cpiA021_control_previous(database, vr_var_name,
                                                                        x_var_name,
                                                                        alpha_value)
  ...
  # Step 03 - Results ----------------------------------------------------------
  output_list$"R_results"$"p01_test" <- fn_cpiA021_code_p01_test(database,  vr_var_name, x_var_name, alpha_value)


  output_list$"R_results"$"p02_plots"$"plot001" <- fn_cpiA021_code_p02_plot001(results_p01_test = output_list$"R_results"$"p01_test")
  output_list$"R_results"$"p02_plots"$"plot002" <- fn_cpiA021_code_p02_plot002(results_p01_test = output_list$"R_results"$"p01_test")
  output_list$"R_results"$"p02_plots"$"plot003" <- fn_cpiA021_code_p02_plot003(results_p01_test = output_list$"R_results"$"p01_test")
  output_list$"R_results"$"p02_plots"$"plot004" <- fn_cpiA021_code_p02_plot004(results_p01_test = output_list$"R_results"$"p01_test")
...
fn_cpiA021_gen02 <- function(database,  vr_var_name, x_var_name, alpha_value){


  all_results <- fn_cpiA021_gen01(database,  vr_var_name, x_var_name, alpha_value)
