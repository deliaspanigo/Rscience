...
module_cpiF005_s01_varselection_ui <- function(id){
...
module_cpiF005_s01_varselection_server <- function(id, input_general){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      control_user_01 <- reactive({
        ...
        validate(
          need(!is.null(input_general), "Error 01: Module Anova 2 Ways (Mixed) s01 - input_general can not be NULL."),
          need(!is.null(vector_var_names_database()), "Error 10: Module Anova 2 Ways (Mixed) s01 - vector_var_names_database() can not be NULL."),
          need(!is.null(intro_source_database()), "Error 11: Module Anova 2 Ways (Mixed) s01 - intro_source_database() can not be NULL.")
        )
      ...
      # # # Var selection for Anova 2 Ways (Mixed)y
        ...
        div(shinyjs::useShinyjs(), id = ns("input-var-selection"),
            ...
            fluidRow(
              column(4,
                     ...
                     fluidRow(
                       column(12,
                              selectInput(inputId = ns("factor01_var_name"), label = "Factor 01 (Fixed Effect)",
                                          choices = set_options,
                                          selected = set_options[1])
                       )),
                     fluidRow(
                       column(12,
                              selectInput(inputId = ns("factor02_var_name"), label = "Factor 02 (Random Effect)",
                                          choices = set_options,
                                          selected = set_options[1])
                       ))
              ),
      ...
      control_user_02 <- reactive({
        ...
        validate(
          need(!is.null(input$vr_var_name), "Error 09: Module Anova 2 Ways (Mixed) s01 - input$vr_var_name can not be NULL."),
          need(!is.null(input$factor01_var_name), "Error 10: Module Anova 2 Ways (Mixed) s01 - input$factor01_var_name can not be NULL."),
          need(!is.null(input$factor02_var_name), "Error 10: Module Anova 2 Ways (Mixed) s01 - input$factor02_var_name can not be NULL."),
          need(!is.null(input$alpha_value), "Error 11: Module Anova 2 Ways (Mixed) s01 - input$alpha_value can not be NULL.")
        )

        validate(
          need(is.vector(input$vr_var_name), "Error 12: Module Anova 2 Ways (Mixed) s01 - input$vr_var_name must be a vector."),
          need(is.vector(input$factor01_var_name), "Error 13: Module Anova 2 Ways (Mixed) s01 - input$factor01_var_name must be a vector."),
          need(is.vector(input$factor02_var_name), "Error 13: Module Anova 2 Ways (Mixed) s01 - input$factor02_var_name must be a vector."),
          need(is.vector(input$alpha_value), "Error 14: Module Anova 2 Ways (Mixed) s01 - input$alpha_value must be a vector.")
        )

        validate(
          need(length(input$vr_var_name) == 1, "Error 15: Module Anova 2 Ways (Mixed) s01 - input$vr_var_name has length 1."),
          need(length(input$factor01_var_name) == 1, "Error 16: Module Anova 2 Ways (Mixed) s01 - input$factor01_var_name has length 1."),
          need(length(input$factor02_var_name) == 1, "Error 16: Module Anova 2 Ways (Mixed) s01 - input$factor02_var_name has length 1."),
          need(length(input$alpha_value) == 1, "Error 17: Module Anova 2 Ways (Mixed) s01 - input$alpha_value has length 1.")
        )


        validate(
          need(input$vr_var_name != "", "Select a response variable."),
          need(input$factor01_var_name != "", "Select a factor01."),
          need(input$factor02_var_name != "", "Select a factor02.")

        )

        validate(
          need(input$vr_var_name != input$factor01_var_name, "Selected variables can not be equal."),
          need(input$vr_var_name != input$factor02_var_name, "Selected variables can not be equal."),
          need(input$factor01_var_name != input$factor02_var_name, "Selected variables can not be equal.")
        )
      ...
      # # Factor selection
      observeEvent(input$factor02_var_name, {
      ...
      factor01_var_name <- reactive({
        req(action_button_show())

        output_value <- input$factor01_var_name
        return(output_value)
      })

      factor02_var_name <- reactive({
        req(action_button_show())

        output_value <- input$factor02_var_name
        return(output_value)
      })
      ...
      output_list <- reactive({
        ...
        the_list <- list(vr_var_name(), factor01_var_name(), factor02_var_name(),alpha_value(), intro_source_database())

        names(the_list) <- c("vr_var_name", "factor01_var_name", "factor02_var_name","alpha_value", "intro_source_database")
        the_list
      })
...
module_cpiF005_s02_rscience_ui <- function(id){
...
module_cpiF005_s02_rscience_server <- function(id, input_general, input_01_anova){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      control_user_01 <- reactive({
        ...
        validate(
          need(!is.null(input_general()), "Error 01: Module Anova 2 Ways (Mixed) s02 - input_general can not be NULL."),
          need(!is.null(input_01_anova()), "Error 02: Module Anova 2 Ways (Mixed) s02 - input_01_anova can not be NULL.")
        )

        check_previous <- fn_cpiF005_control_previous(database = input_general()$database,
      ...
      RR_general <- reactive({
        ...
        the_output <- fn_cpiF005_results(database = input_general()$database,
                                         vr_var_name = input_01_anova()$vr_var_name,
                                         factor01_var_name = input_01_anova()$factor01_var_name,
                                         factor02_var_name = input_01_anova()$factor02_var_name,
                                         alpha_value = input_01_anova()$alpha_value)
      ...
      control_user_02 <- reactive({
        ...
        #
        validate(
          need(!is.null(RR_general()), "Error 03: Module Anova 2 Ways (Mixed) s02 - RR_general() can not be NULL.")
        )
        #
        #
        #
        #
        #         # Control post
        # check_post <- fn_cpiF005_control_post(list_results_from_cpiA001_anova1way = RR_general())
        # #
        # validate(
      ...
      RR_code <- reactive({
        ...
        req(control_user_02())
        the_code <- fn_cpiF005_code_sectionALL(intro_source_database = input_01_anova()$intro_source_database,
                                               vr_var_name = input_01_anova()$vr_var_name,
                                               factor01_var_name = input_01_anova()$factor01_var_name,
                                               factor02_var_name = input_01_anova()$factor02_var_name,
                                               alpha_value = input_01_anova()$alpha_value)
      ...
      output$tab03_analysis_anova_obj02 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        selected_objs <- c("df_factor01_info")
      ...
      output$tab03_02_df_factor01_info <- renderDT({
        ...
        datatable(
          ...
          target = 'row',
          fontSize = "26px"
        ) %>% formatRound(columns= c("median"), digits=4)
      })
      ...
      output$tab03_analysis_anova_obj22_fisher <- renderDT({
      ...
      output$tab03_analysis_anova_obj22_par <- renderDT({
      ...
      output$tab44_analisis01 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("list_lmer_anova")
      ...
      output$tab44_analisis02 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("summary_lmer_anova")
      ...
      output$tab44_analisis03 <- renderPrint({
      ...
      output$tab44_analisis04 <- renderDT({
      ...
      output$tab44_analisis05 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("list_fixed_efect")
      ...
      output$tab03_analysis_anova_FULL_B <- renderUI({
        ...
        div(
          ...
          # h2("2) Factor Summary"),
          # h3("R Object: df_factor01_info"),
          # DTOutput(ns("tab03_02_df_factor01_info")),
          ...
          br(), br(), br(),
          h2("2) Output 01 - Fixed Effect"),
          verbatimTextOutput(ns("tab44_analisis05")),

          br(), br(), br(),
          h2("3) Output 01"),
          verbatimTextOutput(ns("tab44_analisis01")),

          #DTOutput(ns("tab03_analysis_anova_obj22_Bextra")),
          br(), br(), br(),

          h2("4) Output 02"),
          verbatimTextOutput(ns("tab44_analisis02")),
          ...
          h2("5) Output 03"),
          verbatimTextOutput(ns("tab44_analisis03")),
          #DTOutput(ns("tab03_analysis_anova_obj22_par")),
          br(), br(), br(),

          h2("6) Fount of Variance"),
          DTOutput(ns("tab44_analisis04")),
          br(), br(), br()
          # h2("6) Position"),
      ...
      output$tab22_analysis_anova_obj04 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        selected_objs <- c("list_homogeneity_test")
      ...
      output$tab33_factor01_position_levels <- renderDT({
        ...
        # Vector con nombres de elementos a ver
        selected_objs <- c("df_vr_position_levels_factor01")
      ...
      output$tab33_factor01_dispersion_levels <- renderDT({
        ...
        # Vector con nombres de elementos a ver
        selected_objs <- c("df_vr_dispersion_levels_factor01")
      ...
      output$tab33_factor02_position_levels <- renderDT({
        ...
        # Vector con nombres de elementos a ver
        selected_objs <- c("df_vr_position_levels_factor02")
      ...
      output$tab33_factor02_dispersion_levels <- renderDT({
        ...
        # Vector con nombres de elementos a ver
        selected_objs <- c("df_vr_dispersion_levels_factor02")
      ...
      output$tab33_combination_position_levels <- renderDT({
        ...
        # Vector con nombres de elementos a ver
        selected_objs <- c("df_vr_position_levels_combination")
      ...
      output$tab33_combination_dispersion_levels <- renderDT({
        ...
        # Vector con nombres de elementos a ver
        selected_objs <- c("df_vr_dispersion_levels_combiantion")
      ...
      output$page_test <- renderUI({
        ...
        div(
          ...
          shiny::tabsetPanel(id = ns("super_tabset_panel"),
                             ...
                             tabPanel("Summary",
                                      fluidRow(
                                        column(12, h1("Anova 2 Ways (Mixed)"))
                                      ),

                                      fluidRow(
                                        column(12,
                                               h2("1) Position - Factor01"),
                                               h3("R Object: df_vr_position_levels_factor01"),
                                               DTOutput(ns("tab33_factor01_position_levels"))
                                        )
                                      ),
                                      br(), br(), br(),
                                      ...
                                      fluidRow(
                                        column(12,
                                               h2("2) Dispersion - Factor01"),
                                               h3("R Object: df_vr_dispersion_levels_factor01"),
                                               DTOutput(ns("tab33_factor01_dispersion_levels"))
                                        )
                                      ),
                                      br(), br(), br(),
                                      fluidRow(
                                        column(12,
                                               h2("3|) Position - Factor02"),
                                               h3("R Object: df_vr_position_levels_factor02"),
                                               DTOutput(ns("tab33_factor02_position_levels"))
                                        )
                                      ),
                                      br(), br(), br(),
                                      fluidRow(
                                        column(12,
                                               h2("4) Dispersion - Factor02"),
                                               h3("R Object: df_vr_dispersion_levels_factor02"),
                                               DTOutput(ns("tab33_factor02_dispersion_levels"))
                                        )
                                      ),
                                      br(), br(), br(),
                                      ...
                                      fluidRow(
                                        column(12,
                                               h2("5) Position - Combination"),
                                               h3("R Object: df_vr_position_levels_combination"),
                                               DTOutput(ns("tab33_combination_position_levels"))
                                        )
                                      ),
                                      br(), br(), br(),
                                      fluidRow(
                                        column(12,
                                               h2("6) Dispersion - Combination"),
                                               h3("R Object: df_vr_dispersion_levels_combiantion"),
                                               DTOutput(ns("tab33_combination_dispersion_levels"))
                                        )
                                      ),
                                      br(), br(), br()
                             ),
                             tabPanel("Plots - Raw Data",  # 05,
                                      fluidRow(column(12, h1("Anova 2 Ways (Mixed)"))),
                                      fluidRow(
                                        #column(1),
                             ...
                             #          fluidRow(column(12, h1("Anova 2 Ways (Mixed)y"))),
                             #          fluidRow(
                             #            #column(1),
                             #            column(12,
                             ...
                             #          fluidRow(h1("Anova 2 Ways (Mixed)y")),
                             #          fluidRow(
                             #            #column(1),
                             #            column(12,
                             #                   #plotOutput(ns("tab04_plots")),
