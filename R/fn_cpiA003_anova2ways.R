...
fn_cpiA003_TakeCode <- function(selected_fn){
...
fn_cpiA003_ObjNamesInOrder <- function(selected_fn){
...
# # # # Control funcitons
# Control previous
fn_cpiA003_control_previous <- function(database, vr_var_name, factor_var_name, block_var_name, alpha_value){
...
fn_cpiA003_code_p01_test <- function(database, vr_var_name, factor01_var_name,
                                  factor02_var_name, alpha_value){
  ...
  vector_rol_vars <- c("VR", "FACTOR01", "FACTOR02")
  ...
  # # # Anova test
  lm_anova <- lm(VR ~ FACTOR01 + FACTOR02 + FACTOR01:FACTOR02, data = minibase)               # Linear model
  ...
  model_error_var <- df_table_anova$`Mean Sq`[4]
  model_error_sd <- sqrt(model_error_var)

  df_model_error_factor01 <- data.frame(
    "order" = df_factor01_info$order,
    "level" = df_factor01_info$level,
    "n" = df_factor01_info$n,
    "model_error_var" = model_error_var,
    "model_error_sd" = model_error_sd
  )
  df_model_error_factor01["model_error_se"] <- df_model_error_factor01["model_error_sd"]/sqrt(df_model_error_factor01$n)
  df_model_error_factor01


  # # # Standard error from model for each level
  df_model_error_factor02 <- data.frame(
    "order" = df_factor02_info$order,
    "level" = df_factor02_info$level,
    "n" = df_factor02_info$n,
    "model_error_var" = model_error_var,
    "model_error_sd" = model_error_sd
  )
  df_model_error_factor02["model_error_se"] <- df_model_error_factor02["model_error_sd"]/sqrt(df_model_error_factor02$n)
  df_model_error_factor02


  # # # Standard error from model for each level
  df_model_error_combination <- data.frame(
    "order" = df_combination_info$order,
    "level" = df_combination_info$level,
    "n" = df_combination_info$n,
    "model_error_var" = model_error_var,
    "model_error_sd" = model_error_sd
  )
  df_model_error_combination["model_error_se"] <- df_model_error_combination["model_error_sd"]/sqrt(df_model_error_combination$n)
  df_model_error_combination
  ...
  tukey_factor01_full_groups <- agricolae::HSD.test(y = lm_anova,
                                                    trt = "FACTOR01",
                                                    alpha = alpha_value,
                                                    group = TRUE,
                                                    console = FALSE,
  ...
  # # # Tukey test - Factor02
  # Tukey with groups - Full version
  tukey_factor02_full_groups <- agricolae::HSD.test(y = lm_anova,
                                                    trt = "FACTOR02",
                                                    alpha = alpha_value,
                                                    group = TRUE,
                                                    console = FALSE,
                                                    unbalanced = check_unbalanced_reps_factor02)



  # # # Tukey test - Tukey pairs comparation - Full version
  tukey_factor02_full_pairs <- agricolae::HSD.test(y = lm_anova,
                                                   trt = "FACTOR02",
                                                   alpha = alpha_value,
                                                   group = FALSE,
                                                   console = FALSE,
  ...
  # # Original table from R about Tukey
  df_tukey_table_factor02 <- tukey_factor02_full_groups$groups
  df_tukey_table_factor02




  # # # Tukey test - Combination
  # Tukey with groups - Full version
  tukey_combination_full_groups <- agricolae::HSD.test(y = lm_anova,
                                                       trt = c("FACTOR01", "FACTOR02"),
                                                       alpha = alpha_value,
                                                       group = TRUE,
                                                       console = FALSE,
                                                       unbalanced = check_unbalanced_reps_combination)



  # # # Tukey test - Tukey pairs comparation - Full version
  tukey_combination_full_pairs <- agricolae::HSD.test(y = lm_anova,
                                                      trt = c("FACTOR01", "FACTOR02"),
                                                      alpha = alpha_value,
                                                      group = FALSE,
                                                      console = FALSE,
  ...
  # # Original table from R about Tukey
  df_tukey_table_combination <- tukey_combination_full_groups$groups
  df_tukey_table_combination
  ...
  test_residuals_homogeneity <- bartlett.test(residuals ~ COMBINATION, data = minibase_mod)
  ...
  # Tabla plot 001
  df_table_plot001 <- aggregate(VR ~ FACTOR01 + FACTOR02, data = minibase, FUN = mean)
  colnames(df_table_plot001)[3] <- "mean"
  df_table_plot001$"color" <- df_combination_info$color
  ...
  df_table_plot002 <- data.frame(
    ...
    "n" = df_factor01_info$n,
    "mean" = tapply(minibase[,1], minibase[,2], mean),
    "model_error_se" = df_model_error_factor01$model_error_se
  )
  ...
  df_table_plot002["color"] <- df_factor01_info$color

  correct_pos_letters <- order(rownames(df_tukey_table_factor01))
  vector_letters <- df_tukey_table_factor01$group[correct_pos_letters]
  ...
  df_table_plot003 <- data.frame(
    ...
    "n" = df_factor02_info$n,
    "mean" = tapply(minibase[,1], minibase[,3], mean),
    "model_error_se" = df_model_error_factor02$model_error_se
  )

  df_table_plot003["lower_limit"]  <- df_table_plot003$mean - df_table_plot003$model_error_se
  df_table_plot003["upper_limmit"] <- df_table_plot003$mean + df_table_plot003$model_error_se
  df_table_plot003["color"] <- df_factor02_info$color

  correct_pos_letters_f02 <- order(rownames(df_tukey_table_factor02))
  vector_letters_f02 <- df_tukey_table_factor02$group[correct_pos_letters_f02]
  df_table_plot003["group"] <- vector_letters_f02



  # Tabla plot 004
  df_table_plot004 <- data.frame(
    "order" = df_combination_info$order,
    "level" = df_combination_info$level,
    "n" = df_combination_info$n,
    "mean" = tapply(minibase[,1], minibase[,4], mean),
    "model_error_se" = df_model_error_combination$model_error_se
  )

  df_table_plot004["lower_limit"]  <- df_table_plot004$mean - df_table_plot004$model_error_se
  df_table_plot004["upper_limmit"] <- df_table_plot004$mean + df_table_plot004$model_error_se
  df_table_plot004["color"] <- df_combination_info$color

  correct_pos_letters_combination <- order(rownames(df_tukey_table_combination))
  vector_letters_combination <- df_tukey_table_combination$group[correct_pos_letters_combination]
  df_table_plot004["group"] <- vector_letters_combination

  # --- # hide_: Proccesing objects order
  hide_correct_order <- fn_cpiA003_ObjNamesInOrder(selected_fn = fn_cpiA003_code_p01_test)
...
fn_cpiA003_code_p02_plot001 <- function(results_p01_test){
...
fn_cpiA003_code_p02_plot002 <- function(results_p01_test){
...
fn_cpiA003_code_p02_plot003 <- function(results_p01_test){
  ...
  new_plot <-  with(results_p01_test,{
    ...
    plot003 <-   plotly::add_trace(p = plot003,
                                   ...
                                   marker = list(symbol = "line-ew-open",
                                                 ...
                                                 opacity = 1,
                                                 line = list(width = 5)),
                                   error_y = list(type = "data", array = df_table_plot003$model_error_se)
    )
    ...
    plot003 <-  add_text(p = plot003,
                         x = df_table_plot003$level,
                         y = df_table_plot003$mean,
                         text = df_table_plot003$group, name = "Tukey Group",
                         size = 20)

    # # # Title and settings...
    plot003 <- plotly::layout(p = plot003,
                              xaxis = list(title = "FACTOR02"),
                              yaxis = list(title = "VR"),
...
fn_cpiA003_code_p02_plot004 <- function(results_p01_test){
  ...
  new_plot <-  with(results_p01_test,{
    ...
    plot004 <-   plotly::add_trace(p = plot004,
                                   ...
                                   marker = list(symbol = "line-ew-open",
                                                 ...
                                                 opacity = 1,
                                                 line = list(width = 5)),
                                   error_y = list(type = "data", array = df_table_plot004$model_error_se)
    )



    plot004 <-  add_text(p = plot004,
                         x = df_table_plot004$level,
                         y = df_table_plot004$mean,
                         text = df_table_plot004$group, name = "Tukey Group",
                         size = 20)

    # # # Title and settings...
    plot004 <- plotly::layout(p = plot004,
                              xaxis = list(title = "Combination"),
                              yaxis = list(title = "VR"),
                              title = "Plot 004 - Mean y model standard error",
                              font = list(size = 20),
                              margin = list(t = 100))
...
# Control post
fn_cpiA003_control_p01_test <- function(all_results){
...
fn_cpiA003_control_p02_plots <- function(all_results){
...
fn_cpiA003_gen01 <- function(database, vr_var_name, factor01_var_name,
                             factor02_var_name, alpha_value){
  ...
  # Step 01 - R_code
  output_list$"R_code"$"p01_test"    <- fn_cpiA003_TakeCode(selected_fn = fn_cpiA003_code_p01_test)
  output_list$"R_code"$"p02_plots"$"plot001"   <- fn_cpiA003_TakeCode(selected_fn = fn_cpiA003_code_p02_plot001)
  output_list$"R_code"$"p02_plots"$"plot002"   <- fn_cpiA003_TakeCode(selected_fn = fn_cpiA003_code_p02_plot002)
  output_list$"R_code"$"p02_plots"$"plot003"   <- fn_cpiA003_TakeCode(selected_fn = fn_cpiA003_code_p02_plot003)
  output_list$"R_code"$"p02_plots"$"plot004"   <- fn_cpiA003_TakeCode(selected_fn = fn_cpiA003_code_p02_plot004)


  # Step 02 - Pre Control ------------------------------------------------------
  output_list$"check_control"$"previous" <- fn_cpiA003_control_previous(database, vr_var_name,
                                                          factor01_var_name, factor02_var_name,
                                                          alpha_value)
  ...
  # Step 03 - Results ----------------------------------------------------------
  output_list$"R_results"$"p01_test" <- fn_cpiA003_code_p01_test(database, vr_var_name,
                                                                 factor01_var_name,
                                                          factor02_var_name, alpha_value)


  output_list$"R_results"$"p02_plots"$"plot001" <- fn_cpiA003_code_p02_plot001(results_p01_test = output_list$"R_results"$"p01_test")
  output_list$"R_results"$"p02_plots"$"plot002" <- fn_cpiA003_code_p02_plot002(results_p01_test = output_list$"R_results"$"p01_test")
  output_list$"R_results"$"p02_plots"$"plot003" <- fn_cpiA003_code_p02_plot003(results_p01_test = output_list$"R_results"$"p01_test")
  output_list$"R_results"$"p02_plots"$"plot004" <- fn_cpiA003_code_p02_plot004(results_p01_test = output_list$"R_results"$"p01_test")
...
fn_cpiA003_gen02 <- function(database, vr_var_name, factor01_var_name,
                             factor02_var_name, alpha_value){


  all_results <- fn_cpiA003_gen01(database, vr_var_name, factor01_var_name,
                                  factor02_var_name, alpha_value)
  ...
  selection01 <- c("df_selected_vars",
                   "df_table_anova",
                   "df_factor01_info",
                   "check_unbalanced_reps_factor01",
                   "df_tukey_table_factor01",
                   "df_model_error_factor01",
                   "df_factor02_info",
                   "check_unbalanced_reps_factor02",
                   "df_tukey_table_factor02",
                   "df_model_error_factor02",
                   "df_combination_info",
                   "check_unbalanced_reps_combination",
                   "df_tukey_table_combination",
                   "df_model_error_combination")
