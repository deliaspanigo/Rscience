...
module_cpiF004_s01_varselection_ui <- function(id){
...
module_cpiF004_s01_varselection_server <- function(id, input_general){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      control_user_01 <- reactive({
        ...
        validate(
          need(!is.null(input_general), "Error 01: Module Anova 2 Ways (Aleatorized) s01 - input_general can not be NULL."),
          need(!is.null(vector_var_names_database()), "Error 10: Module Anova 2 Ways (Aleatorized) s01 - vector_var_names_database() can not be NULL."),
          need(!is.null(intro_source_database()), "Error 11: Module Anova 2 Ways (Aleatorized) s01 - intro_source_database() can not be NULL.")
        )
      ...
      # # # Var selection for Anova 2 Ways (Aleatorized)y
        ...
        div(shinyjs::useShinyjs(), id = ns("input-var-selection"),
            ...
            fluidRow(
              column(4,
                     ...
                     fluidRow(
                       column(12,
                              selectInput(inputId = ns("factor01_var_name"), label = "Factor 01",
                                          choices = set_options,
                                          selected = set_options[1])
                       )),
                     fluidRow(
                       column(12,
                              selectInput(inputId = ns("factor02_var_name"), label = "Factor 02",
                                          choices = set_options,
                                          selected = set_options[1])
                       ))
                     ),
      ...
      control_user_02 <- reactive({
        ...
        validate(
          need(!is.null(input$vr_var_name), "Error 09: Module Anova 2 Ways (Aleatorized) s01 - input$vr_var_name can not be NULL."),
          need(!is.null(input$factor01_var_name), "Error 10: Module Anova 2 Ways (Aleatorized) s01 - input$factor01_var_name can not be NULL."),
          need(!is.null(input$factor02_var_name), "Error 10: Module Anova 2 Ways (Aleatorized) s01 - input$factor02_var_name can not be NULL."),
          need(!is.null(input$alpha_value), "Error 11: Module Anova 2 Ways (Aleatorized) s01 - input$alpha_value can not be NULL.")
        )

        validate(
          need(is.vector(input$vr_var_name), "Error 12: Module Anova 2 Ways (Aleatorized) s01 - input$vr_var_name must be a vector."),
          need(is.vector(input$factor01_var_name), "Error 13: Module Anova 2 Ways (Aleatorized) s01 - input$factor01_var_name must be a vector."),
          need(is.vector(input$factor02_var_name), "Error 13: Module Anova 2 Ways (Aleatorized) s01 - input$factor02_var_name must be a vector."),
          need(is.vector(input$alpha_value), "Error 14: Module Anova 2 Ways (Aleatorized) s01 - input$alpha_value must be a vector.")
        )

        validate(
          need(length(input$vr_var_name) == 1, "Error 15: Module Anova 2 Ways (Aleatorized) s01 - input$vr_var_name has length 1."),
          need(length(input$factor01_var_name) == 1, "Error 16: Module Anova 2 Ways (Aleatorized) s01 - input$factor01_var_name has length 1."),
          need(length(input$factor02_var_name) == 1, "Error 16: Module Anova 2 Ways (Aleatorized) s01 - input$factor02_var_name has length 1."),
          need(length(input$alpha_value) == 1, "Error 17: Module Anova 2 Ways (Aleatorized) s01 - input$alpha_value has length 1.")
        )
...
module_cpiF004_s02_rscience_ui <- function(id){
...
module_cpiF004_s02_rscience_server <- function(id, input_general, input_01_anova){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      control_user_01 <- reactive({
        ...
        validate(
          need(!is.null(input_general()), "Error 01: Module Anova 2 Ways (Aleatorized) s02 - input_general can not be NULL."),
          need(!is.null(input_01_anova()), "Error 02: Module Anova 2 Ways (Aleatorized) s02 - input_01_anova can not be NULL.")
        )

        check_previous <- fn_cpiF004_control_previous(database = input_general()$database,
      ...
      RR_general <- reactive({
        ...
        the_output <- fn_cpiF004_results(database = input_general()$database,
                                         vr_var_name = input_01_anova()$vr_var_name,
                                         factor01_var_name = input_01_anova()$factor01_var_name,
                                         factor02_var_name = input_01_anova()$factor02_var_name,
                                         alpha_value = input_01_anova()$alpha_value)
      ...
      control_user_02 <- reactive({
        ...
        #
        validate(
          need(!is.null(RR_general()), "Error 03: Module Anova 2 Ways (Aleatorized) s02 - RR_general() can not be NULL.")
        )
        #
        #
        #
        #
        #         # Control post
        # check_post <- fn_cpiF004_control_post(list_results_from_cpiA001_anova1way = RR_general())
        # #
        # validate(
      ...
      RR_code <- reactive({
        ...
        req(control_user_02())
        the_code <- fn_cpiF004_code_sectionALL(intro_source_database = input_01_anova()$intro_source_database,
                                               vr_var_name = input_01_anova()$vr_var_name,
                                               factor01_var_name = input_01_anova()$factor01_var_name,
                                               factor02_var_name = input_01_anova()$factor02_var_name,
                                               alpha_value = input_01_anova()$alpha_value)
      ...
      output$tab44_analisis01 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("list_lme4_anova")
      ...
      output$tab44_analisis02 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("summary_lme_anova")
      ...
      output$page_test <- renderUI({
        ...
        div(
          ...
          shiny::tabsetPanel(id = ns("super_tabset_panel"),
                             ...
                             tabPanel("Summary",
                                      fluidRow(
                                        column(12, h1("Anova 2 Ways (Aleatorized)"))
                                      ),
                             ...
                             tabPanel("Plots - Raw Data",  # 05,
                                      fluidRow(column(12, h1("Anova 2 Ways (Aleatorized)"))),
                                      fluidRow(
                                        #column(1),
                             ...
                             #          fluidRow(column(12, h1("Anova 2 Ways (Aleatorized)y"))),
                             #          fluidRow(
                             #            #column(1),
                             #            column(12,
                             ...
                             #          fluidRow(h1("Anova 2 Ways (Aleatorized)y")),
                             #          fluidRow(
                             #            #column(1),
                             #            column(12,
                             #                   #plotOutput(ns("tab04_plots")),
