...
module_cpiA006_s01_varselection_ui <- function(id){
...
module_cpiA006_s01_varselection_server <- function(id, input_general){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$vars_selection <- renderUI({
        ...
        div(shinyjs::useShinyjs(), id = ns("input-var-selection"),
            fluidRow(
              column(6, h1("Simple Linear Regresion"))
            ),
            fluidRow(
              column(6,
...
module_cpiA006_s02_rscience_ui <- function(id){
...
module_cpiA006_s02_rscience_server <- function(id, input_general, input_01_anova){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      RR_general <- reactive({
        ...
        the_output <- fn_cpiA006_gen02(database = input_general()$database,
                                       vr_var_name = input_01_anova()$vr_var_name,
