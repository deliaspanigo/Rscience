...
module_cpiB002_s01_varselection_ui <- function(id){
...
module_cpiB002_s01_varselection_server <- function(id, input_general){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$vars_selection <- renderUI({
        ...
        div(shinyjs::useShinyjs(), id = ns("input-var-selection"),
            fluidRow(
              column(12, h1("Generalized Linear Modelo - ANOVA 1 Way - Poisson"))
            ),
            fluidRow(
              column(6,
...
module_cpiB002_s02_rscience_ui <- function(id){
...
module_cpiB002_s02_rscience_server <- function(id, input_general, input_01_anova){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      control_user_01 <- reactive({
        ...
        check_previous <- cpiB002_anova1way_poisson_control_previous(database = input_general()$database,
      ...
      RR_general <- reactive({
        ...
        the_output <- cpiB002_anova1way_poisson_results(database = input_general()$database,
                                                vr_var_name = input_01_anova()$vr_var_name,
                                                factor_var_name = input_01_anova()$factor_var_name,
                                                alpha_value = input_01_anova()$alpha_value)
      ...
      control_user_02 <- reactive({
        ...
        #
        #         # Control post
        # check_post <- cpiB002_anova1way_poisson_control_post(list_results_from_cpiA001_anova1way = RR_general())
        # #
        # validate(
      ...
      #
      #   req(control_user_02())
      #   all_tables_g01 <- cpiB002_anova1way_poisson_recruit_g01_Tables(list_results_from_cpiA001_anova1way = RR_general())
      #   all_tables_g01
      # })
      #
      #
      ...
      #
      #   req(control_user_02())
      #   all_plots_g01 <- cpiB002_anova1way_poisson_recruit_g01_FactorPlots(list_results_from_cpiA001_anova1way = RR_general())
      #   all_plots_g01
      # })
      #
      #
      ...
      #
      #   req(control_user_02())
      #   all_tables_g02 <- cpiB002_anova1way_poisson_recruit_g02_Tables(list_results_from_cpiA001_anova1way = RR_general())
      #   all_tables_g02
      # })
      ...
      #
      #   req(control_user_02())
      #   all_plots_g02 <- cpiB002_anova1way_poisson_recruit_g02_ResidualsPlots(list_results_from_cpiA001_anova1way = RR_general())
      #   all_plots_g02
      # })
      ...
      RR_code <- reactive({
        ...
        req(control_user_02())
        the_code <- cpiB002_anova1way_poisson_code_sectionALL(intro_source_database = input_01_anova()$intro_source_database,
                                                      vr_var_name = input_01_anova()$vr_var_name,
                                                      factor_var_name = input_01_anova()$factor_var_name,
                                                      alpha_value = input_01_anova()$alpha_value)
        the_code
      ...
      output$el_plot <- renderPlot({
        ...
        vector_orden <- 1:length(vector_media_VR)
        #Ploteamos las medias de los niveles del factor
        plot(x = vector_orden, y = vector_media_VR,
             xaxt = "n",
             xlim = c(min(vector_orden), max(vector_orden)),
             ylim = c(min(vector_minimo_VR), max(vector_maximo_VR)),
             ylab = "VR",
             xlab = "FACTOR")
      ...
      output$el_plot2 <- renderPlotly({
        ...
        plot002_factor <-   add_trace(p = plot002_factor,
                                      type = "scatter",
                                      mode = "markers",
                                      x = df_resumen03$level,
                                      y = df_resumen03$VR_mean,
                                      color = df_resumen03$level,
                                      colors = df_resumen03$color,
                                      marker = list(symbol = "line-ew-open",
                                                    size = 50,
                                                    opacity = 1,
                                                    line = list(width = 5)),
                                      error_y = list(type = "data",
                                                     array = abs(df_resumen03$VR_upper_error), # $VR_upper_error,
                                                     arrayminus = abs(df_resumen03$VR_lower_error), # VR_lower_error,
                                                     visible = TRUE)
        )
