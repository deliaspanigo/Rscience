...
fn_cpiF004_TakeCode <- function(selected_fn){
...
fn_cpiF004_ObjNamesInOrder <- function(selected_fn){
...
# # # # Control funcitons
# Control previous
fn_cpiF004_control_previous <- function(database, vr_var_name, factor01_var_name, factor02_var_name, alpha_value){
...
# Control post
fn_cpiF004_control_post <- function(list_results_from_fn_cpiF004){

  # if(is.null(list_results_from_fn_cpiF004)){
  #   text_output <- "Control post test 001: Object 'list_results_from_fn_cpiF004' can not be NULL."
  #   check_ok <- FALSE
  ...
  #
  #
  # if(!(obj_name01 %in% names(list_results_from_fn_cpiF004))){
  #   text_output <- "Control post test 002: Object 'df_table_anova' doesn't exist in 'list_results_from_fn_cpiF004'."
  #   check_ok <- FALSE
  #   return(Hmisc::llist(check_ok, text_output))
  # }
  #
  #
  # # # # 1) About the table
  # if(is.null(list_results_from_fn_cpiF004[obj_name01])){
  ...
  #
  #
  # selected_obj01 <- list_results_from_fn_cpiF004[[obj_name01]]
  #
  #
...
fn_cpiF004_results <- function(database, vr_var_name, factor01_var_name, factor02_var_name, alpha_value){
  ...
  list_lme4_anova <- lme4::lmer(RV ~ (1 | FACTOR01) + (1 | FACTOR02) + (1 | FACTOR01:FACTOR02) , data = minibase)

  intervalo_confianza <- suppressMessages(confint(list_lme4_anova,
                                                  oldNames = FALSE,
                                                  level = confidence_value))

  summary_lme_anova <- summary(list_lme4_anova)

  list_anova <- lmerTest::ranova(list_lme4_anova)




  # Extraer las varianzas
  vector_names <- c("FACTOR01", "FACTOR02", "FACTOR01:FACTOR02", "ERROR", "TOTAL")
  df_info <- as.data.frame(lme4::VarCorr(list_lme4_anova))
  ...
  minibase_mod$"residuals" <- residuals(list_lme4_anova)
  ...
  hide_correct_order <- fn_cpiF004_ObjNamesInOrder(selected_fn = fn_cpiF004_results)
...
fn_cpiF004_code_section01_Libreries <- function(){
...
fn_cpiF004_code_section02_FileSource <- function(intro_source_database){
...
fn_cpiF004_code_section03_VarSelection <- function(vr_var_name, factor01_var_name, alpha_value){
...
fn_cpiF004_code_section04_UntilTheEnd <- function(){


  the_code <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_results)

  return(the_code)
}


fn_cpiF004_code_sectionXX_g01_Plots <- function(){



  list_code <- list()

  list_code[[1]] <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_factor_plot001)
  list_code[[2]] <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_factor_plot002)
  list_code[[3]] <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_factor_plot003)
  list_code[[4]] <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_factor_plot004)
  list_code[[5]] <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_factor_plot005)
  list_code[[6]] <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_factor_plot006)
  list_code[[7]] <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_factor_plot007)
...
fn_cpiF004_code_sectionXX_g02_Plots <- function(){


  list_code <- list()

  list_code[[1]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot001)
  list_code[[2]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot002)
  list_code[[3]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot003)
  list_code[[4]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot004)
  list_code[[5]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot005)
  list_code[[6]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot006)
  list_code[[7]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot007)
  list_code[[8]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot008)
  list_code[[9]]  <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot009)
  list_code[[10]] <- fn_cpiF004_TakeCode(selected_fn = fn_cpiF004_residuals_plot010)
  ...
  # objetos <- ls("package:Rscience", pattern = "^fn_cpiF004_residuals_plot+[0-9]+")
  #
  #
  #
  # # Filtrar solo las funciones que coinciden con el patrón
  # #list_plot <- objetos[grep("^fn_cpiF004_plot[0-9]+", objetos)]
  # list_plot <- objetos
  ...
  #
  #
  #   selected_code <- paste0("fn_cpiF004_TakeCode(", x, ")")
...
fn_cpiF004_code_sectionALL <- function(intro_source_database, vr_var_name, factor01_var_name, alpha_value){

  # objetos <- ls()
  #
  # # Filtrar solo las funciones que coinciden con el patrón
  # selected_fun <- objetos[grep("^fn_cpiF004_code_section[0-9]+_[a-zA-Z]", objetos)]
  # selected_fun <- sort(selected_fun)

  vector_code <- list()

  vector_code[1] <- fn_cpiF004_code_section01_Libreries()
  vector_code[2] <- fn_cpiF004_code_section02_FileSource(intro_source_database)
  vector_code[3] <- fn_cpiF004_code_section03_VarSelection(vr_var_name, factor01_var_name, alpha_value)
  vector_code[4] <- fn_cpiF004_code_section04_UntilTheEnd()
  vector_code[5] <- fn_cpiF004_code_sectionXX_g01_Plots()
  vector_code[6] <- fn_cpiF004_code_sectionXX_g02_Plots()
...
fn_cpiF004_recruit_g01_Tables <- function(list_results_from_fn_cpiF004){


  all_tables <- with(list_results_from_fn_cpiF004, {
...
fn_cpiF004_recruit_g02_Tables <- function(list_results_from_fn_cpiF004){
...
# # # Plots FACTOR
fn_cpiF004_factor_plot001 <- function(minibase_mod, df_factor_info){
...
fn_cpiF004_factor_plot002 <- function(df_table_factor_plot002){
...
fn_cpiF004_factor_plot003 <- function(df_table_factor_plot003){
...
fn_cpiF004_factor_plot004 <- function(df_table_factor_plot004){
...
fn_cpiF004_factor_plot005 <- function(minibase_mod, df_table_factor_plot005){
...
fn_cpiF004_factor_plot006 <- function(minibase_mod, df_table_factor_plot006){
...
fn_cpiF004_factor_plot007 <- function(df_table_factor_plot007){
...
fn_cpiF004_recruit_g01_FactorPlots <- function(list_results_from_fn_cpiF004){



  all_plots <-  with(list_results_from_fn_cpiF004, {

    list_plots <- list()

    list_plots[[1]] <- fn_cpiF004_factor_plot001(minibase_mod, df_factor_info)
    list_plots[[2]] <- fn_cpiF004_factor_plot002(df_table_factor_plot002)
    list_plots[[3]] <- fn_cpiF004_factor_plot003(df_table_factor_plot003)
    list_plots[[4]] <- fn_cpiF004_factor_plot004(df_table_factor_plot004)
    list_plots[[5]] <- fn_cpiF004_factor_plot005(minibase_mod, df_table_factor_plot005)
    list_plots[[6]] <- fn_cpiF004_factor_plot006(minibase_mod, df_table_factor_plot006)
    list_plots[[7]] <- fn_cpiF004_factor_plot007(df_table_factor_plot007)
...
fn_cpiF004_residuals_plot001 <- function(minibase_mod, df_factor_info){
...
fn_cpiF004_residuals_plot002 <- function(minibase_mod, df_table_residuals_plot002){
...
fn_cpiF004_residuals_plot003 <- function(minibase_mod, df_table_residuals_plot003){
...
fn_cpiF004_residuals_plot004 <- function(minibase_mod){
...
fn_cpiF004_residuals_plot005 <- function(minibase_mod){
...
fn_cpiF004_residuals_plot006 <- function(minibase_mod, df_factor_info){
...
fn_cpiF004_residuals_plot007 <- function(minibase_mod, df_factor_info){
...
fn_cpiF004_residuals_plot008 <- function(minibase_mod){
...
fn_cpiF004_residuals_plot009 <- function(minibase_mod){
...
fn_cpiF004_residuals_plot010 <- function(minibase_mod){
...
fn_cpiF004_recruit_g02_ResidualsPlots <- function(list_results_from_fn_cpiF004){
  ...
  all_plots <-  with(list_results_from_fn_cpiF004, {
    ...
    list_plots[[1]]  <- fn_cpiF004_residuals_plot001(minibase_mod, df_factor_info)
    list_plots[[2]]  <- fn_cpiF004_residuals_plot002(minibase_mod, df_table_residuals_plot002)
    list_plots[[3]]  <- fn_cpiF004_residuals_plot003(minibase_mod, df_table_residuals_plot003)
    list_plots[[4]]  <- fn_cpiF004_residuals_plot004(minibase_mod)
    list_plots[[5]]  <- fn_cpiF004_residuals_plot005(minibase_mod)
    list_plots[[6]]  <- fn_cpiF004_residuals_plot006(minibase_mod, df_factor_info)
    list_plots[[7]]  <- fn_cpiF004_residuals_plot007(minibase_mod, df_factor_info)
    list_plots[[8]]  <- fn_cpiF004_residuals_plot008(minibase_mod)
    list_plots[[9]]  <- fn_cpiF004_residuals_plot009(minibase_mod)
    list_plots[[10]] <- fn_cpiF004_residuals_plot010(minibase_mod)
