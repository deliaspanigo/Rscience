...
module_cpiA021_s01_varselection_ui <- function(id){
...
module_cpiA021_s01_varselection_server <- function(id, input_general){
  moduleServer(
    id,
...
module_cpiA021_s02_rscience_ui <- function(id){

  ns <- shiny::NS(id)

  div(
    selectInput(inputId = ns("model_selector"), label = "Select Model",
                choices = c("forward" = "forward",
                            "backward" = "backward",
                            "both" = "both")), br(), br(),

    conditionalPanel(condition = "input.model_selector == 'both'",
                     ns = ns, uiOutput(ns("page_test01"))),
    conditionalPanel(condition = "input.model_selector == 'forward'",
                     ns = ns, uiOutput(ns("page_test02"))),
    conditionalPanel(condition = "input.model_selector == 'backward'",
                     ns = ns, uiOutput(ns("page_test03")))
  )
...
module_cpiA021_s02_rscience_server <- function(id, input_general, input_01_anova){
  moduleServer(
    id,
    function(input, output, session) {
      ...
      RR_general <- reactive({
        ...
        the_output <- fn_cpiA021_gen02(database = input_general()$database,
                                       vr_var_name = input_01_anova()$vr_var_name,
      ...
      output$tab01_analysis_obj01 <- renderPrint({
        ...
        nombres_a_ver <- c("df_selected_vars", "phrase01",
                           "phrase02", "phrase03")
      ...
      output$tab01_analysis_obj02 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("list_summary_forward")
      ...
      output$tab01_analysis_obj03 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("df_position_forward")
      ...
      output$tab01_analysis_obj04 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("df_dispersion_forward")

        # Usar lapply para mostrar los elementos deseados
        mi_lista[nombres_a_ver]

      })
      output$tab01_analysis_obj05 <- renderText({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("print_forward_lm")

        # Usar lapply para mostrar los elementos deseados
        elegido <- mi_lista[nombres_a_ver]
        elegido <- unlist(elegido)
        elegido <- paste(elegido, collapse = "\n")
        elegido
      })


      output$tab01_analysis_obj01B <- renderPrint({
      ...
      output$tab01_analysis_obj02B <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("list_summary_backward")

        # Usar lapply para mostrar los elementos deseados
        mi_lista[nombres_a_ver]

      })
      output$tab01_analysis_obj03B <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("df_position_backward")

        # Usar lapply para mostrar los elementos deseados
        mi_lista[nombres_a_ver]

      })
      output$tab01_analysis_obj04B <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("df_dispersion_backward")

        # Usar lapply para mostrar los elementos deseados
        mi_lista[nombres_a_ver]

      })
      output$tab01_analysis_obj05B <- renderText({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("print_backward_lm")
      ...
      output$tab01_analysis_FULL <- renderUI({
        ...
        div(
          ...
          column(6, h2("1) References"),
                 verbatimTextOutput(ns("tab01_analysis_obj01B"))
          )
          ),
          br(), br(), br(),

          fluidRow(
            column(6, h2("2) Forward - Selected Model"),
            verbatimTextOutput(ns("tab01_analysis_obj02"))),
            column(6, h2("2) Backward - Selected Model"),
                   verbatimTextOutput(ns("tab01_analysis_obj02B")))
            ),
          br(), br(), br(),

          fluidRow(
            column(6, h2("3) Forward - Position"),
          verbatimTextOutput(ns("tab01_analysis_obj03"))),
          column(6, h2("3) Backward - Position"),
                 verbatimTextOutput(ns("tab01_analysis_obj03B")))
          ),
          br(), br(), br(),

          fluidRow(
            column(6, h2("4) Forward - Dispersion"),
          verbatimTextOutput(ns("tab01_analysis_obj04"))),
          column(6, h2("4) Backward - Dispersion"),
                 verbatimTextOutput(ns("tab01_analysis_obj04B")))
          ),
          br(), br(), br(),

          fluidRow(
            column(6, h2("5) Forward - Steps"),
          verbatimTextOutput(ns("tab01_analysis_obj05"))),
          column(6, h2("5) Backward - Steps"),
                 verbatimTextOutput(ns("tab01_analysis_obj05B")))
          ),
          br(), br(), br()
        )
      ...
      output$tab01_analysis_FULL_soloF <- renderUI({

        ns <- shiny::NS(id)

        div(
          fluidRow(
            column(12, h2("1) References"),
                   verbatimTextOutput(ns("tab01_analysis_obj01"))
            )
          ),
          br(), br(), br(),
          ...
          fluidRow(
            column(12, h2("2) Forward - Selected Model"),
                   verbatimTextOutput(ns("tab01_analysis_obj02")))
          ),
          br(), br(), br(),

          fluidRow(
            column(12, h2("3) Forward - Position"),
                   verbatimTextOutput(ns("tab01_analysis_obj03")))
          ),
          br(), br(), br(),

          fluidRow(
            column(12, h2("4) Forward - Dispersion"),
                   verbatimTextOutput(ns("tab01_analysis_obj04")))
          ),
          br(), br(), br(),

          fluidRow(
            column(12, h2("5) Forward - Steps"),
                   verbatimTextOutput(ns("tab01_analysis_obj05")))
          ),
          br(), br(), br()
        )
      ...
      output$tab01_analysis_FULL_soloB <- renderUI({
        ...
        div(
          ...
          fluidRow(
            column(12, h2("2) Backward - Selected Model"),
                   verbatimTextOutput(ns("tab01_analysis_obj02B")))
          ),
          br(), br(), br(),

          fluidRow(
            column(12, h2("3) Backward - Position"),
                   verbatimTextOutput(ns("tab01_analysis_obj03B")))
          ),
          br(), br(), br(),

          fluidRow(
            column(12, h2("4) Backward - Dispersion"),
                   verbatimTextOutput(ns("tab01_analysis_obj04B")))
          ),
          br(), br(), br(),

          fluidRow(
            column(12, h2("5) Backward - Steps"),
                   verbatimTextOutput(ns("tab01_analysis_obj05B")))
          ),
          br(), br(), br()
        )
...
#######################################################
      ...
      output$tab02_requerimentsF1 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("test_residuals_normality_forward")

        # Usar lapply para mostrar los elementos deseados
        mi_lista[nombres_a_ver]
      })

      output$tab02_requerimentsF2 <- renderPrint({
      ...
      output$tab02_requerimentsB1 <- renderPrint({
        ...
        # Vector con nombres de elementos a ver
        nombres_a_ver <- c("test_residuals_normality_backward")

        # Usar lapply para mostrar los elementos deseados
        mi_lista[nombres_a_ver]
      })

      output$tab02_requerimentsB2 <- renderPrint({
      ...
      output$tab02_homogeneityplotF1 <- plotly::renderPlotly({
        RR_general()$"out04A_plots_residuals"[[2]]
      })

      output$tab02_homogeneityplotF2 <- plotly::renderPlotly({
        RR_general()$"out04A_plots_residuals"[[2]]
      })

      output$tab02_homogeneityplotB1 <- plotly::renderPlotly({
        RR_general()$"out04A_plots_residuals"[[3]]
      })
      output$tab02_homogeneityplotB2 <- plotly::renderPlotly({
        RR_general()$"out04A_plots_residuals"[[3]]
      })
                             ...
                             tabPanel("Requeriments",
                                      ...
                                      fluidRow(
                                        column(6,
                                               ...
                                               verbatimTextOutput(ns("tab02_requerimentsF1"))
                                        ),
                                        column(6,
                                               h2("1) Requeriment - Normality test - Residuals"),
                                               verbatimTextOutput(ns("tab02_requerimentsB1"))
                                        )
                                      ),
                                      br(), br(), br(),
                                      fluidRow(
                                        column(6,
                                               ...
                                               plotlyOutput(ns("tab02_homogeneityplotF1"), height = "100%", width = "100%")),
                                        column(6,
                                               h2("2) Requeriment - Homogeneity visual evaluation - Residuals"),
                                               plotlyOutput(ns("tab02_homogeneityplotB1"), height = "100%", width = "100%"))
                                      )
                             ),
                             tabPanel("Plots",
                                      fluidRow(
        ...
        div(
          ...
          shiny::tabsetPanel(id = ns("super_tabset_panel"),
                             tabPanel("Analysis",
                                      fluidRow(
                                        column(12,
                                               h1("MLM Selection - Forward"),
                                               uiOutput(ns("tab01_analysis_FULL_soloF"))
                             ...
                             tabPanel("Requeriments",
                                      ...
                                      fluidRow(
                                        column(12,
                                               h2("1) Requeriment - Normality test - Residuals"),
                                               verbatimTextOutput(ns("tab02_requerimentsF2"))
                                        )
                                      ),
                                      br(), br(), br(),
                                      fluidRow(
                                        column(12,
                                               h2("2) Requeriment - Homogeneity visual evaluation - Residuals"),
                                               plotlyOutput(ns("tab02_homogeneityplotF2"), height = "100%", width = "100%"))
        ...
        div(
          ...
          shiny::tabsetPanel(id = ns("super_tabset_panel"),
                             tabPanel("Analysis",
                                      fluidRow(
                                        column(12,
                                               h1("MLM Selection - Backward"),
                                               uiOutput(ns("tab01_analysis_FULL_soloB"))
                             ...
                             tabPanel("Requeriments",
                                      ...
                                      fluidRow(
                                        column(12,
                                               h2("1) Requeriment - Normality test - Residuals"),
                                               verbatimTextOutput(ns("tab02_requerimentsB2"))
                                        )
                                      ),
                                      br(), br(), br(),
                                      fluidRow(
                                        column(12,
                                               h2("2) Requeriment - Homogeneity visual evaluation - Residuals"),
                                               plotlyOutput(ns("tab02_homogeneityplotB2"), height = "100%", width = "100%"))
